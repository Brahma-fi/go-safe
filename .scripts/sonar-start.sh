...
then
  ...
  # docker run --rm -d --name arm64v8/sonarqube -e SONAR_ES_BOOTSTRAP_CHECKS_DISABLE=true -p 9000:9000 sonarqube
  DOCKER_DEFAULT_PLATFORM=linux/arm64 docker run --rm -d --name sonarqube -p 9000:9000 arm64v8/sonarqube
fi
...
sonar-scanner -D sonar.projectKey="go-safe" \
              -D sonar.projectName="go-safe" \
              -D sonar.scm.provider=git \
              -D sonar.sources=. \
              -D sonar.exclusions=".bin/**,contracts/**" \
              -D sonar.tests=./ \
              ...
              -D sonar.github.repository='https://github.com/brahma-fi/go-safe' \
              -D sonar.token="${token}"
